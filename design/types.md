...
* The JSON keyword `true` or the JSON keyword `false` MUST be considered a valid
  boolean.
...
The integer type provides for numeric values which can represented as signed 64-bit integers and
which may require guaranteed precision. In a model, fields of the integer type MAY be nullable and
MAY be optional.
...
* Any other string containing a valid JSON number (i.e., a string with a fractional or exponential
  value) MAY be used for comparative filtering but MUST NOT be considered an exact match.
...
The float type provides for numeric values which cannot represented as signed 64-bit integers and
for which guaranteed precision is not required. In a model, fields of the float type MAY be nullable
and MAY be optional.
...
be returned with greater precision. I.e., if a date/time value is stored internally with
...
A date/time value which case-insensitively matches either of the valid date/time return formats MUST
...
A CRN value MUST be returned as a JSON string.
